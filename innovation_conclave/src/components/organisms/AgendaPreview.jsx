    ...
    return (
        <div ref={containerRef} className="h-screen w-full flex items-center justify-center sticky top-0 px-6 sm:px-12">
            <motion.div
                ...
                className={`relative w-full max-w-5xl h-[450px] md:h-[350px] flex flex-col md:flex-row items-center rounded-[2.5rem] bg-white dark:bg-slate-950 border border-slate-200/80 dark:border-white/10 shadow-[0_30px_60px_rgba(0,0,0,0.12)] dark:shadow-[0_40px_80px_rgba(0,0,0,0.5)] overflow-hidden`}
            >
                ...
                {/* Right Content Area */}
                <div className="w-full md:w-2/3 h-full p-3 md:p-3 flex flex-col justify-center relative bg-white dark:bg-slate-900/50">
                    <div className={`absolute top-0 right-0 w-64 h-64 rounded-full blur-[80px] opacity-10 pointer-events-none ${item.bgLight}`} />

                    <div className="relative z-10">
                        {/* Duration Badge */}
                        <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold border ${item.border} }`}>
                        ...
                        {/* Title & Description */}
                        <h3 className="text-3xl md:text-5xl font-extrabold text-slate-900 dark:text-white tracking-tight mb-4 md:mb-4 leading-tight">
                            {item.title}
                        </h3>
    ...
    return (
        <section className="bg-slate-50 dark:bg-slate-950 transition-colors duration-500 relative z-10">
            ...
            <div className="max-w-7xl mx-auto px-6 md:px-12">
                ...
                >
                    The <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Agenda</span>
                </motion.h2>
                <motion.p
                    initial={{ opacity: 0, y: 20 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    ...
                    className="mt-6 text-xl text-slate-600 dark:text-slate-400 font-medium  mx-auto"
                >
