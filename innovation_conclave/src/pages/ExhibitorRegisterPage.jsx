...
import {
  ...
  Upload, ImageIcon, Lock, Eye, EyeOff, CheckCircle2, Copy, FileText, Loader2, Ticket
} from 'lucide-react';
import { exhibitors } from '../api/client';
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <button
              ...
              className={`flex flex-col items-center gap-3 p-5 rounded-sm border-2 transition-all duration-300 ${data.type === key
                ? "border-orange-300 bg-orange-300 dark:bg-orange-300/20 shadow-lg shadow-orange-300/10"
                : "border-slate-200 dark:border-white/10 hover:border-slate-300 dark:hover:border-white/20 bg-white dark:bg-slate-800"
                }`}
            >
              <Icon className={`w-7 h-7 ${data.type === key ? "text-orange-300 dark:text-orange-300" : "text-slate-400"}`} />
              <span className={`text-sm font-bold ${data.type === key ? "text-orange-300 dark:text-orange-300" : "text-slate-600 dark:text-slate-300"}`}>{label}</span>
            </button>
          ))}
        </div>
      </div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {["1st", "2nd", "3rd", "4th"].map(y => (
                <button
                  ...
                  className={`py-3 rounded-sm text-sm font-bold border-2 transition-all ${data.year === y
                    ? "border-orange-300 bg-orange-300 text-white shadow-lg shadow-orange-300/20"
                    : "border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-300 hover:border-orange-300"
                    }`}
                >
                  {y}
                </button>
...
function StepPoster({ data, setData }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <label className={`group relative flex flex-col items-center justify-center gap-4 border-2 border-dashed rounded-sm p-10 cursor-pointer transition-all duration-300 ${data.poster
        ? "border-orange-300 bg-orange-300 dark:bg-orange-300/10"
        : "border-slate-200 dark:border-white/10 hover:border-orange-300 hover:bg-slate-50 dark:hover:bg-white/5"
        }`}>
        ...
        {data.posterUrl ? (
          <div className="flex flex-col items-center gap-3">
            {data.poster.type.startsWith("image/")
              ? <img src={data.posterUrl} alt="poster preview" className="w-48 h-32 object-cover rounded-sm shadow-md" />
              : <div className="flex items-center gap-2 text-orange-300 dark:text-orange-300 font-bold"><FileText className="w-8 h-8" />{data.poster.name}</div>
            }
            <span className="text-xs text-orange-300 dark:text-orange-300 font-bold">Click to change</span>
          </div>
        ) : (
          <>
            <div className="w-16 h-16 rounded-sm bg-slate-100 dark:bg-white/5 flex items-center justify-center group-hover:bg-orange-300 dark:group-hover:bg-orange-300/20 transition-colors">
              <ImageIcon className="w-8 h-8 text-slate-400 group-hover:text-white transition-colors" />
            </div>
...
function StepPayment({ data, setData }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Fee Banner */}
      <div className="flex items-center justify-between bg-gradient-to-r from-blue-900 to-blue-600 text-white rounded-sm p-5 shadow-lg shadow-orange-300/20">
        <div>
        ...
        <div className="w-16 h-16 bg-white/15 rounded-sm flex items-center justify-center">
      ...
      <div>
        ...
        <label className={`flex items-center gap-4 border-2 border-dashed rounded-sm p-5 cursor-pointer transition-all ${data.paymentProof
          ...
          <div className={`w-11 h-11 rounded-sm flex items-center justify-center shrink-0 ${data.paymentProof ? "bg-emerald-100 dark:bg-emerald-900/20" : "bg-slate-100 dark:bg-white/5"}`}>
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div>
        ...
        <div className={`flex items-center gap-3 px-4 py-3.5 rounded-sm bg-slate-50 dark:bg-slate-800 border-2 focus-within:ring-2 transition-all ${data.confirmPassword && data.password !== data.confirmPassword
          ? "border-red-400 focus-within:ring-red-400/20"
          : "border-slate-200 dark:border-white/10 focus-within:border-orange-300 focus-within:ring-orange-300/20"
          }`}>
          <Lock className="w-4 h-4 text-slate-400 shrink-0" />
          <input
      ...
      {/* Summary */}
      <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-white/10 rounded-sm p-4">
...
function SuccessScreen() {
  return (
    ...
    >
      <div className="w-24 h-24 rounded-sm bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center shadow-xl shadow-emerald-500/20">
...
export default function ExhibitorRegisterPage() {
  ...
  const submitRegistration = async () => {
    ...
    try {
      ...
      fd.append('org_name', data.orgName);
      fd.append('org_type', data.type);
      if (data.year) fd.append('college_year', data.year);
      fd.append('contact_name', data.contact);
      fd.append('email', data.email);
      fd.append('tagline', data.tagline);
      fd.append('password', data.password);
      if (data.poster) fd.append('poster', data.poster);
      if (data.paymentProof) fd.append('payment_proof', data.paymentProof);
      await exhibitors.register(fd);
      setDone(true);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen transition-colors duration-500 pb-16 flex flex-col items-center">
      ...
      <div className="relative w-full h-56 md:h-84 overflow-hidden pt-16">
        <img
      ...
      <div className="w-full max-w-xl px-4 mt-8">
        ...
        <div className="text-center mb-4">
          <p className="text-xs font-bold uppercase tracking-[0.25em] text-orange-300 dark:text-orange-300 mb-1">Set Up Your Booth</p>
        </div>
              ...
              return (
                <React.Fragment key={label}>
                  <div className="flex flex-col items-center gap-1">
                    <div className={`w-8 h-8 rounded-sm flex items-center justify-center text-xs font-extrabold border-2 transition-all duration-300 ${done_ ? "bg-orange-300 border-orange-300 text-white" :
        ...
        <motion.div
          layout
          className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-white/10 rounded-[0.5rem] shadow-2xl dark:shadow-[0_40px_80px_rgba(0,0,0,0.4)] overflow-hidden"
        >
            ...
            {done ? <SuccessScreen /> : (
              <>
                ...
                <div className="flex flex-col gap-3 mt-8">
                  {error && (
                    <p className="text-xs text-red-500 font-bold bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-500/30 rounded-sm px-4 py-2">{error}</p>
                  )}
                  <div className="flex items-center justify-between gap-4">
                    ...
                    >
                      {loading ? (
                        ...
                        <>{step === TOTAL_STEPS ? "Submit Registration" : "Continue"} <ChevronRight className="w-4 h-4" /></>
                      )}
                    </button>
                  </div>
                </div>
