...
export default function AgendaPage() {
  ...
  useEffect(() => {
    agendaAPI.getAll()
      .then(data => {
        // Map backend DB fields to frontend AgendaTimeline expected prop names
        const mapped = data.map(dbItem => ({
          ...dbItem,
          time: dbItem.time_label
        }));
  ...
  const filteredAgenda = agendaData.filter((session) => {
  ...
  return (
    <div className="min-h-screen text-slate-900 dark:text-slate-100 font-sans selection:bg-orange-600 selection:text-white transition-colors duration-300">
      ...
      <section className="relative pt-32 pb-16 md:pt-40 md:pb-24 overflow-hidden">
        ...
        <div className="relative z-10 container mx-auto px-6 text-center max-w-3xl">
          <h1 className="text-4xl md:text-6xl font-extrabold text-slate-900 dark:text-white mb-6 tracking-tight transition-colors duration-300">
            Event <span className="text-orange-300 dark:text-orange-300">Agenda</span>
          </h1>
      ...
      {/* CTA SECTION */}
      <section className="relative z-10 dark:bg-gradient-to-b dark:from-transparent dark:to-[#0b1120] bg-gradient-to-b from-transparent to-slate-200 dark:bg-white/5 py-24 transition-colors duration-300 text-center">
